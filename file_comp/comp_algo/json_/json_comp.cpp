...
compressJSON(
    ...
    //Reading the whole json file into memory(will chunk encode it later)
    if (!input_file.read(input_data.data(), file_size)) {
        std::cerr << "Input file is not read\n";
        return JSONCompError::FILE_READING_F;
    }
    ...
    if (result != Z_OK) {
        ...
        return JSONCompError::ENCODING_F;
    }
    ...
    if (!output_file.write(reinterpret_cast<char *>(compressed_data.data()), compressed_size)) {
        ...
        return JSONCompError::FILE_WRITING_F;
    }
