//
// Created by JOSHI on 25-07-2025.
//
...
std::optional<audio_headers::AiffHeader> parse_aiff_header(std::ifstream& file) {
    ...
    header.comm_size = read_be32(reinterpret_cast<const uint8_t*>(&comm_chunk.comm_size));
    header.num_channels = read_be16(reinterpret_cast<const uint8_t*>(&comm_chunk.num_channels));
    header.num_sample_frames = read_be32(reinterpret_cast<const uint8_t*>(&comm_chunk.num_sample_frames));
    header.sample_size = read_be16(reinterpret_cast<const uint8_t*>(&comm_chunk.sample_size));
